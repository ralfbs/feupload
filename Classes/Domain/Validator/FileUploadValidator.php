        ...
        if ($_FILES['tx_feupload_upload']['tmp_name'][$property][$property]) {
            ...
            if (in_array($ext, explode(',', $extConf['allowedFileExtensions']))) {
                ...
                $this->errors[] = new Tx_Extbase_Validation_Error(
                        Tx_Extbase_Utility_Localization::translate('error.' . $property . 'WrongFileType.message', 'feupload'), 
                        1332941667);
                return false;
            }
        } else {
            $this->errors[] = new Tx_Extbase_Validation_Error(
                    Tx_Extbase_Utility_Localization::translate('error.' . $property . 'Empty.message', 'feupload'), 1332507175);
            return false;
        }
    }
}
