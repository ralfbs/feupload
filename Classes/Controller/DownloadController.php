...
class Tx_Feupload_Controller_DownloadController extends Tx_Extbase_MVC_Controller_ActionController
{
	 ...
	 * @return	void
	 */
	public function injectFrontendUserGroupRepository(Tx_Feupload_Domain_Repository_FrontendUserGroupRepository $frontendUserGroupRepository)
	{
	...
	public function indexAction()
	{
		...
		if( $GLOBALS['TSFE']->fe_user->user ) 
		{
			...
			foreach( $groupIds as $groupId )
			{
				...
				$files = array_merge($files, $this->fileRepository->findByGroup($group)->toArray());
			}
			
			$files = array_merge($files, $this->fileRepository->findByVisibility(-2)->toArray());
			$files = array_merge($files, $this->fileRepository->findByVisibility(0)->toArray());
		}
		else
		{
			$files = array_merge($files, $this->fileRepository->findByVisibility(-1)->toArray());
			$files = array_merge($files, $this->fileRepository->findByVisibility(-0)->toArray());
		}
	...
	public function deleteAction(Tx_Feupload_Domain_Model_File $file)
	{
		if( $file->getDeletable() )
		{
			...
			$this->flashMessageContainer->add(
				Tx_Extbase_Utility_Localization::translate('LLL:EXT:feupload/Resources/Private/Language/locallang.xml:flash.ok.file.deleted.content'),
				Tx_Extbase_Utility_Localization::translate('LLL:EXT:feupload/Resources/Private/Language/locallang.xml:flash.ok.file.deleted.title'),
		...
		{
			$this->flashMessageContainer->add(
				Tx_Extbase_Utility_Localization::translate('LLL:EXT:feupload/Resources/Private/Language/locallang.xml:flash.error.file.not_deleted.content'),
				Tx_Extbase_Utility_Localization::translate('LLL:EXT:feupload/Resources/Private/Language/locallang.xml:flash.error.file.not_deleted.title', array($file->getTitle())),
	 ...
	 * @return	integer				Sorting position (-1, 0, 1)
	 */
	protected function sortFiles($a, $b)
	{
		if( !$this->settings['sorting']['field'] ) $this->settings['sorting']['field'] = 'getTitle';
